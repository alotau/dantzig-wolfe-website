...
Then<CustomWorld>(
  'I see a Ko-fi donation button on every page of the site',
  async function () {
...
Then<CustomWorld>(
  'the Ko-fi button href is {string}',
...
Then<CustomWorld>(
  'the Ko-fi button has target {string}',
  async function (expectedTarget: string) {
    const btn = this.page.locator('[data-kofi-button]')
    await expect(btn).toBeVisible()
...
Then<CustomWorld>(
  'the Ko-fi button rel contains {string}',
  async function (expectedRel: string) {
    const btn = this.page.locator('[data-kofi-button]')
    await expect(btn).toBeVisible()
