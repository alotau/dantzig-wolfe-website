...
Then<CustomWorld>('I see a {string} section', async function (sectionTitle: string) {
  const heading = this.page.locator(
    `h2, h3, section[aria-label], div[data-section]`,
  )
...
Then<CustomWorld>(
  ...
  async function () {
    const contactLink = this.page.locator(
      'a[href^="mailto:"], a[href*="contact"], a[href*="form"]',
    )
...
Then<CustomWorld>(
  'the section clearly labels how to get in touch',
  async function () {
    // The contact section should have visible text explaining how to reach the author
    const contactText = this.page.locator(
      'section, div',
...
Then<CustomWorld>(
  "I see a link to the website's source repository on GitHub",
  async function () {
    const link = this.page.locator(
      'a[href="https://github.com/alotau/dantzig-wolfe-website"]',
    )
...
Then<CustomWorld>(
  ...
  async function () {
    const link = this.page.locator(
      'a[href="https://github.com/alotau/dantzig-wolfe-python"]',
    )
    await expect(link.first()).toBeVisible()
  },
)
...
Then<CustomWorld>(
  'each link opens in a new tab and is labelled descriptively',
  async function () {
    const repoLinks = this.page.locator(
      'a[href*="github.com/alotau"]',
    )
...
Then<CustomWorld>(
  'I see a section explaining why the site was built',
  async function () {
...
Then<CustomWorld>(
  "the section is written in the author's own words",
  async function () {
...
Then<CustomWorld>(
  'the section is at least one paragraph long',
  async function () {
...
Then<CustomWorld>(
  'the navigation bar contains a link to the About page',
  async function () {
...
Then<CustomWorld>(
  'the site footer contains a link to the About page',
  async function () {
